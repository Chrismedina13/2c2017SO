  ...
  */
  ...
  /*Esto es super importante, lo que hace dup2() es duplicar un fd creandome una copia del mismo
    ...
    */
    	...
    	char rutaDelArchivo;
    	scanf("%s",&rutaDelArchivo);
    	char* bloque = obtenerPuntero(rutaDelArchivo);
    	write( pipe_padreAHijo[1],bloque,strlen(bloque));
  ...
  char* rutaAGuardar = "/home/utnso/tp-2017-2c-s1st3m4s_0p3r4t1v0s/FileSystem/tmp/resultado";
  /*Ya esta, tengo mi resultado en buffer ¿Que puedo hacer con eso? Escribirlo en un archivo por ejemplo ;)*/
  FILE* fd = fopen(rutaAGuardar,"w");
  fputs(buffer,fd);
  fclose(fd);
